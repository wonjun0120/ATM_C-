...
void ATMachine::widrawMoney() { 
	...
	if (check == AUTHENTIFICATION_SUCCESS) {	//정보가 참일 시 실행
		balance = (pAcctArray + index)->widraw(id, pwd, money);	//잔액 = 출금하고 변동된 금액
		nMachineBalance -= abs(balance);	//ATM기에 남은 금액, 출금시 잔액이 - 인 경우, ATM기에 돈이 + 되기 때문에 절댓값을 구하여 계산한다.
...
void ATMachine::managerMode() {	//매니저 모드 실행
	...
	if (check == AUTHENTIFICATION_SUCCESS) {	//참일 시 실행
